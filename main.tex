...
\vspace{-0.1cm}
\small In a $p^+$--$n$ junction, the lightly doped side dominates both $W$ and the minority-carrier diffusion current. At high doping, deviations are partly caused by the constant-mobility assumption in the baseline simulation.
\end{column}
\end{columns}
\end{frame}
...
Quantity & Value \\
\midrule
$|\jsc|$ & \SI{0.3084}{\ampere\per\centi\meter\squared} \\
$\voc$ & \SI{0.6574}{\volt} \\
$\jzero$ & \SI{1.142e-10}{\ampere\per\centi\meter\squared} \\
\bottomrule
\end{tabular}
\end{table}

\small The sign of $\jsc$ follows the Driftfusion current convention; the magnitude is reported here for comparison.
\end{column}
\end{columns}
\end{frame}
...
\toprule
Device & $|\jsc|$ & $\voc$ & $W$ \\
 & \si{\milli\ampere\per\centi\meter\squared} & \si{\volt} & \si{\micro\meter} \\
\midrule
p--n baseline & 308.4 & 0.6574 & 0.5206 \\
PIN, $\di=2\,\um$ & 63.2 & 0.5502 & 2.2679 \\
\bottomrule
\end{tabular}
\end{table}
\end{column}
